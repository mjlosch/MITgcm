...
# because of limitations of open_mdsdataset (and cf_decode with large
# times) we have to hack it here
dls = open_mdsdataset(rdir,prefix=['diagsLAYERS'],geometry='llc')
# hopefully, these are the years 1958-2020
myiters = dls.iter[-63:].values
...
ds = open_mdsdataset(rdir,
                     ...
                     iters=myiters[41:],geometry='llc')

dsm = ds.mean(dim='time').compute()
dsm.to_netcdf(os.path.join(postprocdir,"%s_%s.nc"%(rname,mycycle)))

print("read diags2DMonthly: %s"%rdir)
dm = open_mdsdataset(rdir,prefix=['diags2DMonthly'],geometry='llc')
myiters=dm.iter[dm.iter>1000000].values.astype('int64')
dm = open_mdsdataset(rdir,prefix=['diags2DMonthly'], iters=myiters,
                     delta_t=deltat,ref_date=refdate,geometry='llc')
...
ds = open_mdsdataset(rdir,prefix=['diagsTHflx'],geometry='llc')
myiters = ds.iter[-63:].values
ds = open_mdsdataset(rdir,
                     prefix=['diagsTHflx'],
                     iters=myiters,
                     delta_t=deltat,ref_date=refdate,geometry='llc')

...
C     icFrwAtm  :: net fresh-water flux (E-P) from the atmosphere [kg/m2/s] (+=up)
      ...
      Real*8 snowPrc(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 qPrcRnO(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  myTime
      INTEGER myIter
      INTEGER myThid
CEOP
...
C     snowFac   :: snowing refreshing-age factor [units of 1/snowPr]
      ...
      Real*8     iceFrac  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      ...
      Real*8     tFrzOce  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     frzmltMxL(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 ageFac
      Real*8 snowFac
      Real*8 cphm
      Real*8 opFrac, icFrac
      INTEGER i,j
      LOGICAL dBugFlag
...
C-    Initialise
      ...
      ageFac = 1.D0 - thSIce_deltaT/snowAgTime
...
C-    Net seawater freezing (underestimated if there is surf. melting or rain)
          frzSeaWat(i,j) = MAX( -icFrac*frw2oc(i,j), 0.D0 )
     ...
     &           snowHeight(i,j,bi,bj), Qnet(i,j,bi,bj)
        ...
        ELSEIF (hOceMxL(i,j,bi,bj).GT.0.D0) THEN
          icFlxAtm(i,j,bi,bj) = -Qnet(i,j,bi,bj)
          icFrwAtm(i,j,bi,bj) = EmPmR(i,j,bi,bj)
        ELSE
          icFlxAtm(i,j,bi,bj) = 0.D0
          icFrwAtm(i,j,bi,bj) = 0.D0
        ENDIF
       ENDDO
      ENDDO
...
C--    Net fluxes : (only non-zero contribution where frzmltMxL > 0 )
          ...
          frzSeaWat(i,j) = frzSeaWat(i,j) + MAX(-frw2oc(i,j), 0.D0 )
     ...
     &           snowHeight(i,j,bi,bj), Qnet(i,j,bi,bj)
        ...
        IF ( hOceMxL(i,j,bi,bj).GT.0.D0 )
     &    isIceFree(i,j) = iceMask(i,j,bi,bj).LE.0.D0
     &                  .AND.   iceFrac(i,j) .LE.0.D0
        IF ( iceFrac(i,j) .GT. 0.D0 ) THEN
          iceMask(i,j,bi,bj)=iceFrac(i,j)
          IF ( snowHeight(i,j,bi,bj).EQ.0.D0 )

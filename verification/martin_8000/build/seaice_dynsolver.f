...
C     Jacobian-free Newton Krylov solver (Lemieux et al. 2010, 2012)
       ...
       ENDIF
       IF ( SEAICEupdateDamage .AND. .NOT.SEAICEuseIMEX )
...
C     but we are looking for T = k*P = k*press/(1-k)
     ...
     &            *press(i,j,bi,bj)/(1.D0 - tensileStrFac(i,j,bi,bj))
        ...
        ENDDO
       ENDIF
       CALL DIAGNOSTICS_FILL(damage,'SIdamage',0,1 ,0,1,1,myThid)
...
C     I would like to keep it here.
          IF ( SEAICEuseMEB .OR. SEAICEuseEVP ) THEN

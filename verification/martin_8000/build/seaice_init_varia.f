...
C---  Complete initialization
     ...
     &         *EXP(-SEAICE_cStar*(ONE-AREA(i,j,bi,bj)))
         ...
         ENDDO
        ENDDO
        call port_ranarr((sNx+2*Olx)*(sNy+2*Oly)*nSx*nSy,
     &        cohesion)
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx,sNx+OLx
C     no damage means damage = 1
          damage   (I,J,bi,bj) = 1.D0 * maskC(I,J,kSrf,bi,bj)
          damageNm1(I,J,bi,bj) = damage(I,J,bi,bj)
          dCrit    (I,J,bi,bj) = 1.D0
          cohesion (I,J,bi,bj) = SEAICEcohesion
C    &          *( 1.+cohesion(I,J,bi,bj) ) * maskC(I,J,kSrf,bi,bj)
         ENDDO
        ENDDO

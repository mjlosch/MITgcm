...
C     myThid    :: My Thread Id. number
      Real*8     uIce  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     vIce  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER bi,bj
      Real*8     myTime
      INTEGER myIter
      INTEGER myThid
...
C     msgBuf    :: Informational/error message buffer
      ...
      Real*8 uTrIce    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 vTrIce    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 afx       (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 afy       (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 maskOce   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 iceFrc    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 iceVol    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 oldVol    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 iceTmp    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 r_minArea
      Real*8 meanCellArea, areaEpsil, vol_Epsil
...
C     but what matter here is just to have the right order of magnitude.
      ...
      areaEpsil = 1.D-10 * meanCellArea
      vol_Epsil = 1.D-15 * meanCellArea

      r_minArea = 0.D0
      IF ( iceMaskMin.GT.0.D0 ) r_minArea = 1.D0 / iceMaskMin
...
C--   Initialisation (+ build oceanic mask)
      DO j=1-OLy,sNy+OLy
       DO i=1-OLx,sNx+OLx
         ...
         uTrIce(i,j) = 0.D0
         vTrIce(i,j) = 0.D0
         oceFWfx(i,j,bi,bj) = 0.D0
         oceSflx(i,j,bi,bj) = 0.D0
         oceQnet(i,j,bi,bj) = 0.D0
       ENDDO
      ENDDO
...
C-    Calculate ice transports through tracer cell faces.
        DO j=1-OLy,sNy+OLy
         DO i=1-OLx+1,sNx+OLx
          uTrIce(i,j) = uIce(i,j)*dyG(i,j,bi,bj)
     &                *maskOce(i-1,j)*maskOce(i,j)
         ENDDO
        ENDDO
        DO j=1-OLy+1,sNy+OLy
         DO i=1-OLx,sNx+OLx
          vTrIce(i,j) = vIce(i,j)*dxG(i,j,bi,bj)
...
C      and adjust Ice thickness and snow thickness accordingly
        DO j=1,sNy
         DO i=1,sNx
          IF ( iceFrc(i,j) .GT. 1.D0 ) THEN
c           IF ( dBug(i,j,bi,bj) )
            iceMask(i,j,bi,bj)    = 1.D0
...
c           fsalt  (i,j) = fsalt  (i,j) +
            ...
            snowHeight(i,j,bi,bj) = 0.D0
            Qice1     (i,j,bi,bj) = 0.D0
            Qice2     (i,j,bi,bj) = 0.D0
            snowAge   (i,j,bi,bj) = 0.D0
           ENDIF
          ENDIF
         ENDDO
        ENDDO

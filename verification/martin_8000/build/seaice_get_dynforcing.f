...
C     interpolate to U points
     ...
     &          +0.5D0*(uVel(i,j,ks,bi,bj)+uVel(i+1,j,ks,bi,bj))
     &          -0.5D0*(uIce(i,j,bi,bj)+uIce(i+1,j,bi,bj)))
     &          -SIGN(SINWIN, fCori(i,j,bi,bj))*
     &          (vWind(i,j,bi,bj)
     &          +0.5D0*(vVel(i,j,ks,bi,bj)+vVel(i,j+1,ks,bi,bj))
     &          -0.5D0*(vIce(i,j,bi,bj)+vIce(i,j+1,bi,bj))))
     &          +CDAIR(i-1,j)*(COSWIN*
     &          (uWind(i-1,j,bi,bj)
     &          +0.5D0*(uVel(i-1,j,ks,bi,bj)+uVel(i,j,ks,bi,bj))
     &          -0.5D0*(uIce(i-1,j,bi,bj)+uIce(i,j,bi,bj)))
     &          -SIGN(SINWIN, fCori(i-1,j,bi,bj))*
     &          (vWind(i-1,j,bi,bj)
     &          +0.5D0*(vVel(i-1,j,ks,bi,bj)+vVel(i-1,j+1,ks,bi,bj))
     &          -0.5D0*(vIce(i-1,j,bi,bj)+vIce(i-1,j+1,bi,bj))))
     &         )*SIMaskU(i,j,bi,bj)
C     interpolate to V points
            tauy(i,j,bi,bj)= 0.5D0 *
     &         ( CDAIR(i,j)*(SIGN(SINWIN, fCori(i,j,bi,bj))*
     &          (uWind(i,j,bi,bj)
     &          +0.5D0*(uVel(i,j,ks,bi,bj)+uVel(i+1,j,ks,bi,bj))
     &          -0.5D0*(uIce(i,j,bi,bj)+uIce(i+1,j,bi,bj)))
     &          +COSWIN*
     ...
     &          +0.5D0*(vVel(i,j,ks,bi,bj)+vVel(i,j+1,ks,bi,bj))
     &          -0.5D0*(vIce(i,j,bi,bj)+vIce(i,j+1,bi,bj))))
     &          +CDAIR(i,j-1)*(SIGN(SINWIN, fCori(i,j-1,bi,bj))*
     &          (uWind(i,j-1,bi,bj)
     &          +0.5D0*(uVel(i,j-1,ks,bi,bj)+uVel(i+1,j-1,ks,bi,bj))
     &          -0.5D0*(uIce(i,j-1,bi,bj)+uIce(i+1,j-1,bi,bj)))
     &          +COSWIN*
     &          (vWind(i,j-1,bi,bj)
     &          +0.5D0*(vVel(i,j-1,ks,bi,bj)+vVel(i,j,ks,bi,bj))
     &          -0.5D0*(vIce(i,j-1,bi,bj)+vIce(i,j,bi,bj))))
     &         )*SIMaskV(i,j,bi,bj)
           ENDDO
          ENDDO
         ELSE
...
C     interpolate to U points
     ...
     &          -SIGN(SINWIN, fCori(i  ,j,bi,bj))*vWind(i  ,j,bi,bj) )
     &          + CDAIR(i-1,j)*(
     &          COSWIN                            *uWind(i-1,j,bi,bj)
     &          -SIGN(SINWIN, fCori(i-1,j,bi,bj))*vWind(i-1,j,bi,bj) )
     &         )*SIMaskU(i,j,bi,bj)
C     interpolate to V points
            tauy(i,j,bi,bj)=0.5D0 *
     &         (  CDAIR(i,j  )*(
     &          SIGN(SINWIN, fCori(i,j  ,bi,bj))*uWind(i,j  ,bi,bj)
     &          +COSWIN*vWind(i,j  ,bi,bj) )
     &          + CDAIR(i,j-1)*(
     &          SIGN(SINWIN, fCori(i,j-1,bi,bj))*uWind(i,j-1,bi,bj)

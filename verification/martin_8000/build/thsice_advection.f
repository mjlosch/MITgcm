...
C [N,S,E,W]_edge :: true if N,S,E,W edge of myTile is an Edge of the cube
      Real*8 maskLocC(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      ...
      INTEGER i,j,k
      Real*8 uCfl    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 vCfl    (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C-     Compute CFL number
     ...
     &          MAX( uTrans(i,j), 0.D0 )*recip_rA(i-1,j,bi,bj)
     &         +MAX(-uTrans(i,j), 0.D0 )*recip_rA( i ,j,bi,bj)
     &                               )
           ENDDO
          ENDDO
         ELSE
          DO j=1-OLy,sNy+OLy
           DO i=2-OLx,sNx+OLx
            uCfl(i,j) = deltaTadvect*(
     &        MAX( uTrans(i,j), 0.D0 )/MAX( iceVol(i-1,j), iceEps )
     &       +MAX(-uTrans(i,j), 0.D0 )/MAX( iceVol( i ,j), iceEps )
     &                               )
           ENDDO
          ENDDO
         ENDIF
...
C-     Compute CFL number
     ...
     &          MAX( vTrans(i,j), 0.D0 )*recip_rA(i,j-1,bi,bj)
     &         +MAX(-vTrans(i,j), 0.D0 )*recip_rA(i, j ,bi,bj)
     &                               )
           ENDDO
          ENDDO
         ELSE
          DO j=2-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            vCfl(i,j) = deltaTadvect*(
     &        MAX( vTrans(i,j), 0.D0 )/MAX( iceVol(i,j-1), iceEps )
     &       +MAX(-vTrans(i,j), 0.D0 )/MAX( iceVol(i, j ), iceEps )
     &                               )
           ENDDO
          ENDDO
         ENDIF

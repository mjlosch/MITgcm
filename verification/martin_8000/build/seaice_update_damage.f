...
C     betaFacU/V     :: analog betaFacP1U/V
      ...
      Real*8 sig12C        (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)

      Real*8 MC_FAILURE, TENSILE_FAILURE, fldAtZ, fldAtC
...
C     parameters
      recip_healingTime= 0.
      IF ( SEAICEhealingTime .GT. 0.D0 )
     &     recip_healingTime = 1.D0/SEAICEhealingTime
      recip_damageTime = 0.
      IF ( SEAICEdamageTime .GT. 0.D0 )
     &     recip_damageTime  = 1.D0/SEAICEdamageTime
...
C
          dCrit(I,J,bi,bj) = 1.D0*maskInC(I,J,bi,bj)
         ENDDO
        ENDDO
...
C     principle components
          sig1   = 0.5D0 * ( sigp + sigTmp )
          sig2   = 0.5D0 * ( sigp - sigTmp )
...
CML     &         sig1 .GT. 0.D0 .AND. sig2 .GT. sigmaT(I,J) )
...
C
      CALL EXCH_XY_RL ( damage,  myThid  )
CML      CALL EXCH_XY_RL ( seaice_sigma1 ,  myThid  )
...
CML      CALL EXCH_XY_RL ( seaice_sigma12,  myThid  )
      ...
      Real*8 FUNCTION MC_FAILURE ( sig1, sig2, q, sigC, damageCrit )
      IMPLICIT NONE
C     compute Mohr-Coulomb failure criterion and updage damageCrit
      Real*8 sig1, sig2, q, sigC, damageCrit
      Real*8 denom, rdenm
      denom = sig2 - q*sig1
      rdenm = 0.D0
      IF ( denom .NE. 0.D0 ) rdenm = 1./denom
...
C     compute tensile failure criterion and updage damageCrit
      Real*8 sig1, sig2, sigT, damageCrit
      tensile_failure = damageCrit
      IF ( sig1 .GT. 0.D0 .AND. sig2 .GT. sigT )
...
CML     &     + fldIn(I,J-1)**2 + fldIn(I-1,J-1)**2 ) )
      ...
      Real*8 FUNCTION fldAtC ( fldIn, i, j, bi,bj )
      IMPLICIT NONE
C $Header: /u/gcmpack/MITgcm/verification/offline_exf_seaice/code/SIZE.h,v 1.4 2012/12/08 00:34:29 jmc Exp $
C $Name:  $
...
C     myThid :: my Thread Id. number
      Real*8 damage   (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 recip_rlx(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      INTEGER bi, bj
      Real*8     myTime
      INTEGER myIter
      INTEGER myThid
...
C                        undamaged appararent viscosity
      INTEGER i, j
      Real*8     recip_relaxTime
CEOP

C     zero means zero viscosity, hence elasto-brittle wihout the
C     Maxwell term
      recip_relaxTime = 0.D0
      IF ( SEAICEviscosity .GT. 0.D0 )
...
C     myThid :: my Thread Id. number
      Real*8 damage  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 sigPlus (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 sigMinus(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 sigma12 (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     i,j :: loop indices
      INTEGER i,j
      Real*8 sig11(0:sNx,0:sNy)
      Real*8 sig22(0:sNx,0:sNy)
      Real*8 sig12(1:sNx+1,1:sNy+1)
      Real*8 recip_rlx(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 denom, sumNorm, deltaTloc
      Real*8 fldAtZ
      EXTERNAL fldAtZ
CEOP
...
C     (1 + dt*Elasiticy/viscosity)
      DO j=0,sNy
       DO i=0,sNx
        denom = 1.D0 / (1.D0 + deltaTloc*recip_rlx(I,J))
        sig11(I,J) = 0.5D0 * denom *
     &       ( sigPlus(I,J,bi,bj)+sigMinus(I,J,bi,bj) )
        sig22(I,J) = 0.5D0 * denom *
     &       ( sigPlus(I,J,bi,bj)-sigMinus(I,J,bi,bj) )
       ENDDO
      ENDDO
      DO J=1,sNy+1
...
CML     &       +     maskInC(I,J-1,bi,bj)+maskInC(I-1,J-1,bi,bj)
CML        IF ( sumNorm.GT.0.D0 ) sumNorm = 1.D0 / sumNorm
CML        denom = 1.D0/(1.D0+deltaTloc*sumNorm *
CML     &       ( recip_rlx(I,J  ) + recip_rlx(I-1,J  )
CML     &       + recip_rlx(I,J-1) + recip_rlx(I-1,J-1) ) )
        denom = 1.D0/(1.D0+deltaTloc*fldAtZ(recip_rlx,i,j,bi,bj) )
...
C     i,j :: loop indices
      ...
      Real*8 sig22   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig12   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 sig12C  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,bi,bj)
CEOP
...
C     i,j :: loop indices
      ...
      Real*8 recip_rlx(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 denom, sumNorm, deltaTloc
      Real*8 SEAICEmebMu, SEAICEmebLambda
      Real*8 zetaLoc, etaLoc
      Real*8 sigp, sigm, ep, em, rhs12
      Real*8 fldAtZ, fldAtC
...
CML        sig12C(I,J,bi,bj) = denom * ( deltaTLoc*rhs12 + 0.25D0 *
...
C     recover sigma11 and sigma22
        sig11(I,J) = 0.5D0 * ( sigp+sigm )
        sig22(I,J) = 0.5D0 * ( sigp-sigm )
       ENDDO
      ENDDO
CML      DO J=1,sNy+1
CML       DO I=1,sNx+1
...
CML     &          + press0(I,J-1,bi,bj) + press0(I-1,J-1,bi,bj) )
        ...
        rhs12 = 2.D0 * etaLoc * e12(I,J,bi,bj)
        denom = 1.D0/(1.D0+deltaTloc*fldAtZ(recip_rlx,i,j,bi,bj) )
CML        denom = 1.D0/(1.D0+deltaTloc*sumNorm *
CML     &          ( recip_rlx(I,J  ) + recip_rlx(I-1,J  )
CML     &          + recip_rlx(I,J-1) + recip_rlx(I-1,J-1) ) )
CML        denom = ( 1.D0/(1.D0+deltaTloc*recip_rlx(I  ,J  ) )
CML     &       +    1.D0/(1.D0+deltaTloc*recip_rlx(I-1,J  ) )
CML     &       +    1.D0/(1.D0+deltaTloc*recip_rlx(I  ,J-1) )
CML     &       +    1.D0/(1.D0+deltaTloc*recip_rlx(I-1,J-1) )
CML     &       )*sumNorm
...
CML     &       )
CML        rhs12 = 2.D0 * etaLoc * e12(I,J,bi,bj) * deltaTLoc

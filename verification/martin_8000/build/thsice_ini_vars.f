...
C-        fluxes :
          DO j=1-OLy,sNy+OLy
           DO i=1-OLx,sNx+OLx
            sHeating(i,j,bi,bj) = 0.D0
            flxCndBt(i,j,bi,bj) = 0.D0
            siceAlb(i,j,bi,bj)  = 0.D0
            icFlxSW (i,j,bi,bj) = 0.D0
...
C     Tf = -mu_Tf*salinity = -1.70 deg C;  Qice1 ~ 3.2e5;  Qice2 ~ 3.4e5
         ...
         DO bj = myByLo(myThid), myByHi(myThid)
          DO bi = myBxLo(myThid), myBxHi(myThid)
           DO j=1-OLy,sNy+OLy
            DO i=1-OLx,sNx+OLx
             IF (iceMask(i,j,bi,bj) .NE. 0.D0) THEN
     ...
     &             + cpIce *(Tmlt1-Tf) + Lfresh*(1.D0-Tmlt1/Tf)
      ...
      CALL EXCH_XY_RL ( Tsrf,    myThid )
      CALL EXCH_XY_RL ( Tice1,   myThid )
      CALL EXCH_XY_RL ( Tice2,   myThid )
      CALL EXCH_XY_RL ( Qice1,   myThid )
      CALL EXCH_XY_RL ( Qice2,   myThid )
      CALL EXCH_XY_RL ( snowAge, myThid )

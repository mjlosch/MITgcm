...
C     myThid      :: my Thread Id number
      ...
      INTEGER iMin, iMax
      INTEGER jMin, jMax
      Real*8     icFlag  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     hSnow1   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     tsfCel  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     flxExcSw(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
...
C     turbulent temperature scale [K]
      Real*8 tstar (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     turbulent humidity scale  [kg/kg]
      Real*8 qstar (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     friction velocity [m/s]
      Real*8 ustar (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C     neutral, zref (=10m) values of rd
      Real*8 rdn   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 rd    (1-OLx:sNx+OLx,1-OLy:sNy+OLy) ! = sqrt(Cd)          [-]
      Real*8 rh    (1-OLx:sNx+OLx,1-OLy:sNy+OLy) ! = Ch / sqrt(Cd)     [-]
      Real*8 re    (1-OLx:sNx+OLx,1-OLy:sNy+OLy) ! = Ce / sqrt(Cd)     [-]
C     specific humidity difference [kg/kg]
      Real*8 delq  (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 deltap(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
C
      Real*8 ssq   (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 ren, rhn           ! neutral, zref (=10m) values of re, rh
      Real*8 usn, usm           ! neutral, zref (=10m) wind-speed (+limited)
...
C     stability parameter at zwd [-] (=z/Monin-Obuklov length)
      ...
      Real*8 tau                ! surface stress coef = rhoA * Ws * sqrt(Cd)
      Real*8 tmpbulk
...
c     Real*8       exf_BulkqSat
c     external  exf_BulkqSat
c     Real*8       exf_BulkCdn
c     external  exf_BulkCdn
c     Real*8       exf_BulkRhn
...
C
      DO j=jMin,jMax
       DO i=iMin,iMax
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
        IF ( dBug(i,j,bi,bj) .AND. (icFlag(i,j).GT.0.D0) )
...
C--   Use atmospheric state to compute surface fluxes.
        IF ( (icFlag(i,j).GT.0.D0) .AND.
     &       (atemp(i,j,bi,bj).NE.0.D0) ) THEN
         IF ( hSnow1(i,j).GT.3.D-1 ) THEN
...
C     for ssq and recomputing Ts2 lateron
         ...
         dflwupdT    = emiss*stefanBoltzmann*Ts2*Tsf * 4.D0
...
C-    Large&Pond_1981 code:
           xsq    = MAX(SQRT(ABS(exf_one - huol*16.D0)),exf_one)
           x      = SQRT(xsq)
...
C-    Large&Pond_1981 code:
           xsq    = MAX(SQRT(ABS(exf_one - htol*16.D0)),exf_one)

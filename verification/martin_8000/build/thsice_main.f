...
C     qPrcRn    :: Energy content of Precip+RunOff (+=down) [W/m2]
      ...
      INTEGER iMin, iMax
      INTEGER jMin, jMax
      Real*8 prcAtm(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 snowPr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 qPrcRn(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
c     Real*8 evpAtm(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
c     Real*8 flxAtm(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
c     Real*8 flxSW (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 tauFac
      INTEGER grpDiag
...
C--   and stressReduction is always set to zero
        IF ( stressReduction.GT. 0.D0 ) THEN
          DO j = 1-OLy,sNy+OLy-1
           DO i = 2-OLx,sNx+OLx-1
            tauFac = stressReduction
     &             *(iceMask(i-1,j,bi,bj)+iceMask(i,j,bi,bj))*0.5D0
            fu(i,j,bi,bj) = (1.D0 - tauFac)*fu(i,j,bi,bj)
           ENDDO
          ENDDO
          DO j = 2-OLy,sNy+OLy-1
           DO i = 1-OLx,sNx+OLx-1
            tauFac = stressReduction
     &             *(iceMask(i,j-1,bi,bj)+iceMask(i,j,bi,bj))*0.5D0
            fv(i,j,bi,bj) = (1.D0 - tauFac)*fv(i,j,bi,bj)
           ENDDO
          ENDDO
        ENDIF
